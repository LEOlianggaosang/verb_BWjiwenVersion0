...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_INTENSITY, DP_TYPE_ENUM},
  {DPID_ULTRASOUND_CLEANING, DP_TYPE_BOOL},
  {DPID_WORK_MODE, DP_TYPE_ENUM},
  {DPID_STATUS, DP_TYPE_ENUM},
  {DPID_APPOINTMENT_TIME, DP_TYPE_VALUE},
  {DPID_REMAIN_TIME, DP_TYPE_VALUE},
  {DPID_CLEAN_STATE, DP_TYPE_ENUM},
  {DPID_TEMP_CURRENT, DP_TYPE_VALUE},
  {DPID_WATER_SET, DP_TYPE_VALUE},
  {DPID_WATER_ONCE, DP_TYPE_VALUE},
  {DPID_CONSUMPTION_ONCE, DP_TYPE_VALUE},
  {DPID_WASH_NUMBER_SET, DP_TYPE_ENUM},
  {DPID_WATER_TOTAL, DP_TYPE_VALUE},
  {DPID_CONSUMPTION_TOTAL, DP_TYPE_VALUE},
...
void all_data_update(void)
{
    ...
    mcu_dp_bool_update(DPID_STOP,当前停止); //BOOL型数据上报;
    mcu_dp_enum_update(DPID_INTENSITY,当前清洗强度); //枚举型数据上报;
    mcu_dp_bool_update(DPID_ULTRASOUND_CLEANING,当前超声洗); //BOOL型数据上报;
    mcu_dp_enum_update(DPID_WORK_MODE,当前工作模式); //枚举型数据上报;
    mcu_dp_enum_update(DPID_STATUS,当前设备状态); //枚举型数据上报;
    mcu_dp_value_update(DPID_APPOINTMENT_TIME,当前预约时间); //VALUE型数据上报;
    mcu_dp_value_update(DPID_REMAIN_TIME,当前剩余时间); //VALUE型数据上报;
    mcu_dp_enum_update(DPID_CLEAN_STATE,当前洗涤状态); //枚举型数据上报;
    mcu_dp_value_update(DPID_TEMP_CURRENT,当前当前温度); //VALUE型数据上报;
    mcu_dp_value_update(DPID_WATER_SET,当前水位设置); //VALUE型数据上报;
    mcu_dp_value_update(DPID_WATER_ONCE,当前单次水量); //VALUE型数据上报;
    mcu_dp_value_update(DPID_CONSUMPTION_ONCE,当前单次电量); //VALUE型数据上报;
    mcu_dp_enum_update(DPID_WASH_NUMBER_SET,当前洗涤量设置); //枚举型数据上报;
    mcu_dp_value_update(DPID_WATER_TOTAL,当前耗水总量); //VALUE型数据上报;
    mcu_dp_value_update(DPID_CONSUMPTION_TOTAL,当前耗电总量); //VALUE型数据上报;
...
/*****************************************************************************
函数名称 : dp_download_intensity_handle
功能描述 : 针对DPID_INTENSITY的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_intensity_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char intensity;
    
    intensity = mcu_get_dp_download_enum(value,length);
    switch(intensity) {
        case 0:
        break;
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_enum_update(DPID_INTENSITY, intensity);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_ultrasound_cleaning_handle
功能描述 : 针对DPID_ULTRASOUND_CLEANING的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_ultrasound_cleaning_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:off/1:on
    unsigned char ultrasound_cleaning;
    
    ultrasound_cleaning = mcu_get_dp_download_bool(value,length);
    if(ultrasound_cleaning == 0) {
        //bool off
    }else {
        //bool on
    }
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_bool_update(DPID_ULTRASOUND_CLEANING,ultrasound_cleaning);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_appointment_time_handle
功能描述 : 针对DPID_APPOINTMENT_TIME的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_appointment_time_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long appointment_time;
    
    appointment_time = mcu_get_dp_download_value(value,length);
    /*
    ...
    */
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_value_update(DPID_APPOINTMENT_TIME,appointment_time);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_water_set_handle
功能描述 : 针对DPID_WATER_SET的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_water_set_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long water_set;
    
    water_set = mcu_get_dp_download_value(value,length);
    /*
    //VALUE type data processing
    ...
    */
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_value_update(DPID_WATER_SET,water_set);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_wash_number_set_handle
功能描述 : 针对DPID_WASH_NUMBER_SET的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_wash_number_set_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char wash_number_set;
    
    wash_number_set = mcu_get_dp_download_enum(value,length);
    switch(wash_number_set) {
        case 0:
        break;
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_enum_update(DPID_WASH_NUMBER_SET, wash_number_set);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        case DPID_INTENSITY:
            //清洗强度处理函数
            ret = dp_download_intensity_handle(value,length);
        break;
        case DPID_ULTRASOUND_CLEANING:
            //超声洗处理函数
            ret = dp_download_ultrasound_cleaning_handle(value,length);
        break;
        case DPID_WORK_MODE:
            //工作模式处理函数
            ret = dp_download_work_mode_handle(value,length);
        break;
        case DPID_APPOINTMENT_TIME:
            //预约时间处理函数
            ret = dp_download_appointment_time_handle(value,length);
        break;
        case DPID_WATER_SET:
            //水位设置处理函数
            ret = dp_download_water_set_handle(value,length);
        break;
        case DPID_WASH_NUMBER_SET:
            //洗涤量设置处理函数
            ret = dp_download_wash_number_set_handle(value,length);
        break;
